...
- Dashboard shell (`(dashboard)/layout.tsx`): desktop sidebar + mobile drawer nav (`src/components/dashboard/`), header with breadcrumb + user menu, role-based nav items (Supporter/Creator/Admin — see `nav-items.ts`) driven by `session.user.role`. The overview page (`/dashboard`) shows role-appropriate `StatTile`s — the same 4 summary tiles as the admin Analytics page for admins, campaign totals for creators, contribution totals for supporters — reusing each role's own dedicated page's exact `queryKey`/`queryFn` so TanStack Query shares cache instead of re-fetching.
...
- Admin UI: `/dashboard/admin/users` (list + change role via a native `<select>`), `/dashboard/admin/campaigns` (list all + delete, reusing `CampaignCard`), and `/dashboard/admin/analytics` (stat tiles, a raised-over-time area chart, a raised-by-category bar chart, and a Top Campaigns grid) — all gated by a shared `(dashboard)/dashboard/admin/layout.tsx` that shows "You don't have access to this page" for non-admins. Users/campaigns pages needed no new server endpoints (Better Auth's own admin plugin routes + the existing `GET`/`DELETE /campaigns/:id`); the analytics page is backed by a dedicated admin-only `GET /analytics` aggregation endpoint on the server. The two charts (`src/components/dashboard/charts/`) are plain SVG built per the project's `dataviz` skill — no charting library — using the skill's validated 8-hue categorical palette for the 7 campaign categories (fixed order, matching `landing/categories.tsx`) and a hover crosshair/tooltip layer on both.
